...
type SessionType = 'aspersion' | 'phyto' | 'ndvi'
...
const ndviSchema = z.object({
  program_id: z.string().uuid(),
  // session_date es opcional: el importador la rellena desde el CSV ('Conjunto de datos').
...
function NdviForm({
  ...
  const {
    ...
    resolver: zodResolver(ndviSchema),
    defaultValues: { program_id: programaId },
  })

  const mutation = useMutation({
    mutationFn: async (values: NdviValues) => {
      const body = {
        program_id: values.program_id,
      ...
      if (error) {
        if (typeof error === 'object' && error !== null) {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          applyDrfErrors(error as any, setError, NDVI_FIELDS)
          throw new Error('Corrige los errores del formulario')
        }
        throw new Error('No se pudo crear la sesión de NDVI')
      }
      return data
    },
    onSuccess: () => {
  ...
  return (
    <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          <Label htmlFor="nd-end">Fin estimado</Label>
          <Input id="nd-end" type="date" {...register('est_finish_date')} />
        </div>
      </div>
