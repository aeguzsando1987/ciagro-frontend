    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Crea una organización raíz y asigna a su propietario.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Field label="Dueño de organización *" error={errors.owner_id?.message}>
                  ...
                  <SelectContent>
                    {owners.map((u) => (
                      <SelectItem key={u.id} value={u.id}>
                        {u.username}{' '}
                        {u.individual?.first_name
                          ? `— ${u.individual.first_name} ${u.individual.last_name ?? ''}`.trim()
                          : ''}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              El <strong>dueño (owner)</strong> es la persona responsable de la organización. Debe
              ser usuario con rol Gerente o Admin.
            </p>
          </Field>
          <DialogFooter>
