...
type ModalFrame =
  ...
  | { type: 'sesion'; sesionId: string; sesionType: 'aspersion' | 'phyto' | 'soil_map'; hijoId: string; masterId: string }
...
const taskManagerSearchSchema = z.object({
  ...
  openSesionType: z.enum(['aspersion', 'phyto', 'soil_map']).optional().catch(undefined),
})
...
function HijoModalWrapper({
  ...
  onClose: () => void
  onBack: () => void
  onNavigateSesion: (ref: { sesionId: string; sesionType: 'aspersion' | 'phyto' | 'soil_map' }) => void
}) {
